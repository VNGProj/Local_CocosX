...
float hardShadow(Sampler _sampler, vec4 _shadowCoord, float _bias)
{
	...
	return step(texCoord.z-_bias, DecodeDepth(texture2D(_sampler, texCoord.xy).rgb ) );
#else
...
{
	float fade = min(max(0.11,u_lightInfo.w),1.0);
	...
	float t = when_gt(fragPosDistance,lightShadowFade) * when_lt(fragPosDistance,u_lightShadowCascaded[0]) * ((fragPosDistance - lightShadowFade ) / max(0.001, u_lightShadowCascaded[0] - lightShadowFade));
